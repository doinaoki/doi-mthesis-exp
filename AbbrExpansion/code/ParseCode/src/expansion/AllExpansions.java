	...
	private static void handleExtend() {
		...
		for (ClassInfo classInfo : classInfos) {
			...
			for (Field field : fields) {
				AllExpansions.addExpansion(className.id, field.id, "ClassName", "fields");
			...
			for (MethodName methodName : methodNames) {
				AllExpansions.addExpansion(className.id, methodName.id, "ClassName", "methods");
	...
	private static void handleSiblings(ArrayList<MethodName> mList, ArrayList<Field> fList) {
		...
		while(members.size() > 0) {
			...
			for (TypedIdentifier sibling : members) {
				addExpansion(member.id, sibling.id, member.type, "siblings");
				addExpansion(sibling.id, member.id, member.type, "siblings");
			}
		}
	}
			...
			if (idToClassName.get(id) instanceof ClassName) {
				...
				if (parents != null) {
					for (String pId : parents) {
						AllExpansions.addExpansion(id, pId, "ClassName", "parents");
					}
				...
				for (String ssId : subsubclass) {
					AllExpansions.addExpansion(id, ssId, "ClassName", "subsubclass");
				}
			}
...
//		ancestor <- parentsのすべての親クラス
	...
	private static void toFile() {
		...
		sb.append("id,")
				...
				.append("assignment,")
				.append("methodInvocated,")
				.append("parameterArgument,")
				.append("parameter,")
...
//					System.err.println("not equal size of parameters and arguments " + parameters.size() + " " + arguments.size());
				...
				for (int j = 0; j < parameters.size(); j++) {
					...
					for (int k = 0; k < argument.identifiers.size(); k++) {
						...
						if (!idToClassName.containsKey(identifier.id)) {
							addExpansion(parameter.id, identifier.id, "ParameterName", "argument");
						}
					}
				}
				for (int j = 0; j < methodInvocationInfo.arguments.size(); j++) {
					...
					for (int k = 0; k < argument.identifiers.size(); k++) {
						Identifier identifier = argument.identifiers.get(k);
						if (!idToClassName.containsKey(identifier.id)) {
							addExpansionAccordingToType(identifier.id, methodInvocationInfo.methodName.id, "methodInvocated");
							addExpansionAccordingToType(identifier.id, parameter.id, "parameterArgument");
						}
					}
				}
			} else {
				unresolved.add(methodId);
//				System.err.println("Can not find method declaration of: " + methodId);
			}
		}
//		System.err.println("unresolved method list:");
			...
			if (right != null) {
				...
				for (Identifier identifier1 : left) {
					for (Identifier identifier2 : right) {
						addExpansionAccordingToType(identifier1.id, identifier2.id, "assignment");
						addExpansionAccordingToType(identifier2.id, identifier1.id, "assignment");
					}
				}
			}
		}
